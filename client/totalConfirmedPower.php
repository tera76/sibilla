		...
		"parameters": {
			...
			"data": {
				"request": [{
					...
					"from": "getDate",
					"parameters": {
						"query": "select timestamp, value from syb_alarms_history where data like '%total_population%' order by id desc limit 1000 "
   				}
				}]
			}
		}
...
$scale_value = intval($gold_date_value[$i][1] / 10000000);


echo "<br>" . $gold_date_value[$i][0] ." " . $gold_date_value[$i][1] . " " . $scale_value	  ;

for ($val=700; $val<=  $scale_value ; $val++  ) {
	echo ".";
}
	echo "*";
